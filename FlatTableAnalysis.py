...
class FlatTableAnalysis:
    ...
    def __init__(self, 
        ...
        """
        assert isinstance(df.columns, pd.core.indexes.base.Index), 'header has more than one line'
        ...
        self.col_to_unique = {col: self.df.loc[:, col].drop_duplicates().shape[0] for col in self.df}.get
        ...
        self._df = self.df  # save df version without factorize, need for self.show_header_info()
        self.df = (self.df
                   .drop_duplicates()
                   .apply(lambda ser: ser.factorize()[0])
                   .pipe(lambda obj: obj if isinstance(obj, pd.DataFrame) else obj.to_frame())
                   .assign(**{col: lambda df, col=col: df[col].astype('category') for col in self.df})
                )
        
    def _delete_one_one_relations(self, 
                                  df: pd.DataFrame,
                                 ) -> pd.DataFrame:
        r"""
        ...
        for cols in it.combinations(df, r=2):
            if self.col_to_unique(cols[0]) == self.col_to_unique(cols[1]) == df.loc[:, cols].drop_duplicates().shape[0]:
                edge_list.append(cols)
                
        # if e.g. 4 cols have one-to-one relations between them -> need do delete all but one from them
        G = nx.Graph()
        G.add_edges_from(edge_list)
        to_delete = [sorted(cc, key=self.col_pos)[1:] for cc in nx.connected_components(G)]
    ...
    def get_candidate_keys(self, 
        ...
        pbar = tqdm(total=sum(math.comb(self.df.shape[1], col_num) for col_num in col_nums))
        result = []
        for col_num in col_nums:
            for col_names in it.combinations(self.df, r=col_num):
                result.append([col_names, self.df.loc[:, col_names].drop_duplicates().shape[0]])
                pbar.update(1)
        pbar.close()  
        ...
        return  (
                    ...
                    .pipe(lambda df: df.insert(1, 'col_names_len', df['col_names'].str.len()) or df)
                    .sort_values(['uniques_frac', 'col_names_len'], ascending=[False, True])
                    .reset_index(drop=True)
                    .pipe(lambda df: df.assign(col_names = lambda df: 
                                                   df['col_names'].map(lambda el: tuple(sorted(el, key=self.col_pos)))
                                              )
                         )
                )
        
    def show_fd_graph(self, 
        ...
        for cols in it.combinations(self.df, r=2):
            table.append((cols[0], cols[1], self.df.loc[:, cols].drop_duplicates().shape[0]))
    ...
    def get_density_table(self) -> pd.DataFrame:
        ...
        return (
                    pd.DataFrame(result)
                    .assign(density = lambda df: 
                                df['total_unique'] / (df['left_side_unique'] * df['right_side_unique'])
                           )
                    .sort_values('density', ascending=False)
                    .reset_index(drop=True)
               )
    
    def show_set_relation(self, 
        ...
        CC_all = (sorted(CC_all, key=lambda el: el[0], reverse=True)[:20] + (['...'] if len(CC_all) > 20 else []) + 
                  sorted(CC_all, key=lambda el: el[1], reverse=True)[:20] + (['...'] if len(CC_all) > 20 else [])
                 )
        
        zero_symbol = -1
        try: L_nan_cc = nx.node_connected_component(G, tuple([0] + [zero_symbol] * L_n))
        except KeyError: L_nan_cc = []
        cc_flag = [node[0] for node in L_nan_cc]
        L_nan_L_count = cc_flag.count(0)
        L_nan_R_count = cc_flag.count(1)
        
        try: R_nan_cc = nx.node_connected_component(G, tuple([1] + [zero_symbol] * R_n))
        except KeyError: R_nan_cc = []
    ...
    def get_cols_determinants(self, 
        ...
        pbar = tqdm(total=sum(math.comb(len(other_cols), col_num) for col_num in col_nums))
        result = []
        for col_num in col_nums:
